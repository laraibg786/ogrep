...
// TestIntegrationSearchOrchestrator builds a real ods fixture on disk
// and runs it through the actual SearchOrchestrator (walker + registry
// + matcher + this extractor), asserting matches come back with
// correctly rendered Sheet:Cell Location.Human strings across two
// sheets.
func TestIntegrationSearchOrchestrator(t *testing.T) {
	data := buildOds(t, "application/vnd.oasis.opendocument.spreadsheet", map[string]string{
		"content.xml": wrapContent(
			`<table:table table:name="Budget"><table:table-row>` +
				`<table:table-cell><text:p>irrelevant</text:p></table:table-cell>` +
				`<table:table-cell><text:p>needle value</text:p></table:table-cell>` +
				`</table:table-row></table:table>` +
				`<table:table table:name="Notes"><table:table-row>` +
				`<table:table-cell><text:p>another needle</text:p></table:table-cell>` +
				`</table:table-row></table:table>`),
	})

	dir := t.TempDir()
	fixturePath := filepath.Join(dir, "book.ods")
	...
	for _, m := range sink.matches {
		if m.Format != "ods" {
			t.Errorf("match Format = %q, want %q", m.Format, "ods")
		}
	...
	want := []string{"Budget:B1", "Notes:A1"}
	...
	data := buildOds(t, "application/vnd.oasis.opendocument.spreadsheet", map[string]string{
		"content.xml": wrapContent(
			`<table:table table:name="Sheet1"><table:table-row>` +
				`<table:table-cell><text:p>cat</text:p></table:table-cell>` +
				`<table:table-cell><text:p>cot</text:p></table:table-cell>` +
				`<table:table-cell><text:p>cut</text:p></table:table-cell>` +
				`<table:table-cell><text:p>dog</text:p></table:table-cell>` +
				`</table:table-row></table:table>`),
	})
